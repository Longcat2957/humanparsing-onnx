...
class ATR(ONNXWrapper):
    def __init__(
        ...
        Args:
            ...
            repo_id: Hugging Face의 모델 저장소 ID
            filename: 모델 파일 이름
            model_path: 로컬 모델 경로
        """
    ...
    def infer(self, image: np.ndarray) -> np.ndarray:
        """
        이미지에 대한 인체 파싱 추론을 수행합니다.

        Args:
            image: 입력 이미지 (BGR 순서의 numpy 배열)

        Returns:
            np.ndarray: 각 픽셀이 인체 부위 클래스 ID를 나타내는 분할 마스크
        """
        self.logger.info("Running inference on ATR model")
        # ONNX 모델에 입력할 데이터 전처리
        ...
        # 출력 형태:
        # [
        # <class 'numpy.ndarray'> (1, 18, 128, 128) - 첫 번째 출력
        # <class 'numpy.ndarray'> (1, 18, 128, 128) - 두 번째 출력(파싱 마스크)
        # <class 'numpy.ndarray'> (1, 2, 128, 128) - 세 번째 출력
        # ]
    ...
    def postprocess(
        ...
        output = outputs[1][0]  # (18, 128, 128) 형태의 파싱 마스크 추출
        output = np.transpose(output, (1, 2, 0))  # (128, 128, 18) 채널 순서 변경
        ...
        # 로짓 변환 적용
        logits_result = self._transform_logits(output, meta)
        parsing_result = np.argmax(logits_result, axis=2)  # (h, w) 형태의 클래스 레이블
        parsing_result = np.pad(
            parsing_result, pad_width=1, mode="constant", constant_values=0
        )

        # 의류 부분의 구멍을 채우는 작업 수행
        # 팔 마스크 생성 (클래스 14, 15)
